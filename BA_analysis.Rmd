...
p1 <- df_long_hbo %>% 
  ...
  scale_x_discrete(
    name = '', labels = c('auditory cort.', 'visual cort.')
  ) +
  labs(
    title = 'A) Signal across ROI & trial', 
...
```{r individual traces plots ROI, fig.width=5, fig.height = 4}
...
```{r individual traces plots STIM, fig.width = 3.5}
# STIM på x-aksen
 ...
 p11 <- df_long_hbo %>% 
  ...
  #coord_cartesian(ylim = c(0, 2)) + 
 scale_x_discrete(
    name = '', labels = c('auditory trials', 'visual trials')
  ) +
  labs(
    title = 'C) Mean signal per subject', 
...
```{r new epochs/trial_no column}
# random effect for aud and vis stim
df_long2_hbo$trial_no <- rep(
  1:8, 
  times= 114,
  each = length(unique(df_long2_hbo$cha_name)))

df_long2_hbo <- df_long2_hbo %>% 
  mutate(
...
m1 <- lmer(theta ~ ROI + (1|subject) + (1|cha_name), df_long2_hbo, REML = F)

m2 <- lmer(theta ~ ROI + trial + (1|subject) + (1|cha_name), df_long2_hbo, REML = F)

m3 <- lmer(theta ~ ROI + trial + intensity + (1|subject) + (1|cha_name), df_long2_hbo, REML = F)

# H1
m4 <- lmer(theta ~ ROI*trial + intensity + (1|subject) + (1|cha_name), df_long2_hbo, REML = F)

m5 <- lmer(theta ~ ROI*trial + intensity*ROI + (1|subject) + (1|cha_name), df_long2_hbo, REML = F)

# H1 + H2
m6 <- lmer(theta ~ ROI*trial + intensity*ROI + trial*intensity + (1|subject) + (1|cha_name), df_long2_hbo, REML = F)
