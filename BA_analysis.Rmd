...
df_long_hbo %>% 
  ...
  labs(
    title = 'Hbo signal across ROIs and type of trial',
    y = 'theta (AU)') + 
...
df_long_hbr %>% 
  ...
  labs(
    title = 'Hbr signal across ROIs and type of trial',
    y = 'theta (AU)') + 
  scale_color_discrete(
    name = '',
    labels = c(
...
df_long_baseline_hbr %>% 
  ...
  stat_summary(
    ...
    stat_summary(fun.data = "mean_se", geom = 'errorbar', width = .1) +
    stat_summary(fun = 'mean', geom = 'line', color = 'gray55', aes(group = trial))+ 
  coord_cartesian(ylim = c(-.1, .1)) # læg mærke til skalaen
```
...
m4 <- lmer(theta ~ trial + intensity*ROI + (1|subject) + (1|cha_name), df_long2_int_hbo, REML = F)

m5 <- lmer(theta ~ trial*intensity + ROI*intensity + (1|subject) + (1|cha_name), df_long2_int_hbo, REML = F)

m6 <- lmer(theta ~ trial*intensity + intensity*ROI + trial*ROI + (1|subject) + (1|cha_name), df_long2_int_hbo, REML = F)

m7 <- lmer(theta ~ intensity*trial*ROI + (1|subject) + (1|cha_name), df_long2_int_hbo, REML = F)
