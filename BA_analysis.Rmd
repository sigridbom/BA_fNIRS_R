...
```{r testing}
file = read_csv('data_frames/df_aud_0.csv')

file_name = 'data_frames/df_aud_0.csv'
...
# get means per column and make dataframe wide
file <- file %>% 
  colMeans() %>% 
  as.data.frame()
...
# make column with ID (aud/vis + no)
test <- file %>% mutate(
  subject = str_extract(file_name, "\\d+")
  contrast = str_extract
)

test2 = str_extract(file_name, "\\df([/s/S]*)")
test2 = str_extract(file_name, "\\b\\w")
test2 = str_extract(file_name, starts_with("\\df_"))
...
## H1 auditory vs visual - real data
```{r pivot longer}
df_long <- df %>% 
  pivot_longer(
...
# Rearranging dataframe - BASELINE
## H1 auditory vs visual baseline
```{r pivot longer baseline}
...
```{r}
test <- str_contains(test_long$cha_name, 'hbr')
test
```


```{r}
# example of pivot_longer - to be used shortly
